...
- **v0.3.1** ← *here*: `[dependencies]` table declares the runtime dep on `amalgame-crypto >= 0.3.0`. amc >= 0.8.39 auto-pulls it on `amc package add tls`. Older amc still works — the user just has to `amc package add crypto` manually first. Drops the surprising `Unknown symbol 'JwsKey'` resolver error early adopters hit on the first run after `amc package add tls`
